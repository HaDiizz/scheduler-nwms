    ...
    try:
        ...
        if not accessPoint or accessPoint is None:
                            ...
                            "extensions": {
                                "services_with_info": [
                                    [item["extensions"]["description"],
                                        state, '', f"Accesspoint: online"]
                                ]
                            }
                        })
                    else:
                        group_suffix = item['extensions'].get(
                            'description')[-3:]
                        if group_suffix[-1] == 'A':
                            group_prefix = item['extensions'].get('description')[
                                :-2]
                        else:
                            group_prefix = item['extensions'].get('description')[
                                :-3]
            ...
            if response:
                accessPoint_in_db = []
                accessPoint_now = []
                get_accessPoint_down(response, month, year,
                                     accessPoint_in_db, accessPoint_now)
                            ...
                            if len(accessPoint_list_ids) > 0:
                                ...
                                for item_id in accessPoint_list_ids:
                                    if isinstance(item_id, ObjectId):
                                        ...
                                        accessPoint_all_ids.append(
                                            ObjectId(item_id.id))
                                query = models.AccessPointList.objects(
                                    id__in=accessPoint_all_ids)
...
def get_accessPoint_down(response, month, year, accessPoint_down_in_db, accessPoint_down_now):
    ...
    filter_accessPoint_down = [
        accessPoint for accessPoint in accessPoint_down_in_db if accessPoint not in accessPoint_down_now]
    ...
    models.AccessPointDown.objects(
        accessPoint_id__in=filter_accessPoint_down).delete()
