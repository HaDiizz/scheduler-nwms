    ...
    try:
        ...
        if not service:
            ...
            if response:
                service_in_db = []
                service_now = []
                get_service_down(response, month, year,
                                 service_in_db, service_now)
                            ...
                            if len(service_list_ids) > 0:
                                ...
                                for item_id in service_list_ids:
                                    if isinstance(item_id, ObjectId):
                                        ...
                                        service_all_ids.append(
                                            ObjectId(item_id.id))
                                query = models.ServiceList.objects(
                                    id__in=service_all_ids)
...
def get_service_down(response, month, year, servicedown_in_db, servicedown_now):
    ...
    filter_service_down = [
        service for service in servicedown_in_db if service not in servicedown_now]
